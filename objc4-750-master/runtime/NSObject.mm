...
//      m[0] = M0 = 0xA1A1A1A1
    ...
    bool check() const {// 0 == strncmp((char *)&m[1], M1, M1_len) 成立 表示这个已经被析构了
        return (m[0] == M0 && 0 == strncmp((char *)&m[1], M1, M1_len));
    }

    bool fastcheck() const {
#if CHECK_AUTORELEASEPOOL
        return check();///  返回false 说明该结构体已经被析构了 true表示还存在
#else
        return (m[0] == M0);
#endif
    }
...
#   define POOL_BOUNDARY nil
    ...
    /*
     ...
     分配的内存大小PAGE_MAX_SIZE为4096，即虚拟内存一页大小
...
//        PROT_NONE //页不可访问
    ...
    AutoreleasePoolPage(AutoreleasePoolPage *newParent) //创建函数
    ...
    { 
        if (parent) {
            parent->check();
            assert(!parent->child);
            parent->unprotect();///相当于加锁的操作，x用完之后再进行解锁
            parent->child = this;
            parent->protect();///
        }
        protect();
    ...
    ~AutoreleasePoolPage() //j析构函数
    {
        check();///释放掉
        unprotect();///已经用完了可以进行读写了
        assert(empty());
    ...
    void busted(bool die = true) ///将资源释放掉
    {
        magic_t right;
    ...
    void check(bool die = true) ///检查当前线程是不是这个线程不是则销毁 释放掉
    {
    ...
    {
#if CHECK_AUTORELEASEPOOL
        check(die);///  返回false 说明该结构体已经被析构了 true表示还存在
#else
        if (! magic.fastcheck()) {///  返回false 说明该结构体已经被析构了 true表示还存在
            busted(die);///如果不存在则销毁
        }
#endif
    }
    ...
    id * begin() { ///将这个 指向下一个 this + this所占的内存字节数
        return (id *) ((uint8_t *)this+sizeof(*this));
    }

    id * end() { ///将这个 指向下一个 this + SIZE的大小
        return (id *) ((uint8_t *)this+SIZE);
    }

    bool empty() { ///当next指针指向begin的时候说明这个是空的
        return next == begin();
    }

    bool full() { ///当next指针指向end的时候说明这个是空的
        return next == end();
    }

    bool lessThanHalfFull() {///判断内存中数据是否过半
        return (next - begin() < (end() - begin()) / 2);
    }

    id *add(id obj)///在page中添加一个对象
    {
        assert(!full());///如果满了就报错
        unprotect();///将内存解锁
        id *ret = next;  // faster than `return next-1` because of aliasing 混叠
        *next++ = obj; ///将obj添加到next 地址，然后指向下一个地址
        protect();///将内存加锁
        return ret; ///返回添加对象的地址
    }

    void releaseAll() ///释放全部对象
    {
        releaseUntil(begin());
    }

    void releaseUntil(id *stop) ///传入一个终止的地址量
    {
        // Not recursive: we don't want to blow out the stack 
        // if a thread accumulates a stupendous amount of garbage
        
        while (this->next != stop) {///循环释放page中的对象
            // Restart from hotPage() every time, in case -release 
            // autoreleased more objects
            AutoreleasePoolPage *page = hotPage();

            // fixme I think this `while` can be `if`, but I can't prove it
            ///如果多个都是空的话那么用if应该就不太好使了吧
            while (page->empty()) {
                page = page->parent;
                setHotPage(page);
            }
            ...
            page->unprotect();///解锁
            id obj = *--page->next;///因为next指针指向的为下一个为空的地址，所以首先要--才能是要释放对象的地址
            ...
            if (obj != POOL_BOUNDARY) {///如果不为空的话n，那就释放掉
                objc_release(obj);
            }
        }
    ...
    void kill() 
    {
        ...
        while (page->child) page = page->child;///找到最底层的page

        AutoreleasePoolPage *deathptr;///声明一个变量
        do {
            deathptr = page;
            page = page->parent;
            if (page) {///如果前驱结点存在
                page->unprotect();//解锁
                page->child = nil;//释放掉后继节点
                page->protect();//加锁
            }
            delete deathptr;///删除后面的page
        } while (deathptr != this);
    }
    ...
    static void tls_dealloc(void *p) 
    {
        ...
        // reinstate TLS value while we work///重新恢复数据 将p 设置为hotpage
    ...
    static AutoreleasePoolPage *pageForPointer(uintptr_t p) 
    {
        ...
        assert(offset >= sizeof(AutoreleasePoolPage));///如果offset大于page的所占内存 报错
        //这里的pageForPointer是传入的指针，例如page->begin(),然后在函数内部，会通过整除SIZE=4096,来找到一页的开始的指针，最后返回
    ...
    static inline bool haveEmptyPoolPlaceholder()///判断当前page是否为空
    {
        id *tls = (id *)tls_get_direct(key);
        return (tls == EMPTY_POOL_PLACEHOLDER);
    }

    static inline id* setEmptyPoolPlaceholder()///push了一个pool没有添加对象，为placeHodle
    {
    ...
    static inline AutoreleasePoolPage *hotPage() ///如果存在hotpage就返回hotpage否则 返回nil
    {
        AutoreleasePoolPage *result = (AutoreleasePoolPage *)tls_get_direct(key);///如果存在page的u话就h返回page的地址获取
//        #   define EMPTY_POOL_PLACEHOLDER ((id*)1)
        if ((id *)result == EMPTY_POOL_PLACEHOLDER) return nil;///可用性判断
    ...
    {
        if (page) page->fastcheck();///在autoreloease page上返回 这个page的地址
//        typedef pthread_key_t tls_key_t;  每个线程只有一个这样的key，也就是说每个线程只有一个hotpage，将page设置成hotpage
        tls_set_direct(key, (void *)page);///设置
    }

    static inline AutoreleasePoolPage *coldPage() ///返回当前page的根父类？
    {
        AutoreleasePoolPage *result = hotPage();///codepage 就是hotpage的根父类
        if (result) {
            while (result->parent) {
    ...
    static inline id *autoreleaseFast(id obj)
    {
        ...
        if (page && !page->full()) {///当page存在且不为空的时候 page添加obj,将next指针指向下一个
            return page->add(obj);
        } else if (page) {
            return autoreleaseFullPage(obj, page);///当page存在且为空的时候添加page
        } else {
            return autoreleaseNoPage(obj);///当没有page的时候就设定为placeHodle添加一个
        }
    }

    static __attribute__((noinline))
    id *autoreleaseFullPage(id obj, AutoreleasePoolPage *page)///page满了
    {
        // The hot page is full. page 满了就找下一个page 添加到下一个page里面
        ...
        do {
            if (page->child) page = page->child;///如果page后继节点存在 page 将下一个节点赋值给page ，如果不存在，则创建一个新的page
            else page = new AutoreleasePoolPage(page);
        } while (page->full());

        setHotPage(page);///将当前page赋值为hotpage
        return page->add(obj);///返回z添加obj的地址
    }

    static __attribute__((noinline))
    id *autoreleaseNoPage(id obj)///不存在page
    {   //nopage存在的两种情况 ， 当前没有pooln被push，或者push了但是pool是空的
        // "No page" could mean no pool has been pushed
        // or an empty placeholder pool has been pushed and has no contents yet
        assert(!hotPage());

        bool pushExtraBoundary = false;
        if (haveEmptyPoolPlaceholder()) {///判断当前page是否为空
        ...
        AutoreleasePoolPage *page = new AutoreleasePoolPage(nil);///插入第一个page
        setHotPage(page);///把当前page设置为hotpage
        
        // Push a boundary on behalf of the previously-placeholder'd pool.
        if (pushExtraBoundary) {///如果存在placeHoder 则添加一个boundary 处理这时候e的边界问题，添加一个POOL_BOUNDARY
            page->add(POOL_BOUNDARY);
        }
        
        // Push the requested object or pool.
        return page->add(obj);///添加到pool中将对象
    }
    ...
    id *autoreleaseNewPage(id obj)
    {
        ...
        if (page) return autoreleaseFullPage(obj, page); ///当前的page存在
        else return autoreleaseNoPage(obj);//////当前的page不存在
    }
...
public:
    ...
    static inline void pop(void *token)///传入的page的next指针的值
    {
        AutoreleasePoolPage *page;
        id *stop;

        if (token == (void*)EMPTY_POOL_PLACEHOLDER) {///如果一个page只是被push但是没有添加任何的对象的时候这个EMPTY_POOL_PLACEHOLDER作为一个占位符来占用，类似于swift中的写时复制
            // Popping the top-level placeholder pool.
            if (hotPage()) {///当前线程存在可用的pool
                // Pool was used. Pop its contents normally.
                // Pool pages remain allocated for re-use as usual.
                pop(coldPage()->begin());
            } else {
                // Pool was never used. Clear the placeholder.
                ///没有存在任何可用的pool，将当前的pool清理掉
                setHotPage(nil);
            }
            return;
        }

        page = pageForPointer(token);///返回token page的指针 通过取余 4096 然后 - 余数  找到page的开始地址
        stop = (id *)token; ///传入的为终止地址
        if (*stop != POOL_BOUNDARY) { // nil
            if (stop == page->begin()  &&  !page->parent) {/// 当前指针为page的 开始地址，并且 parent 不存在，所以这个是根page
    ...
    static void printHiwat()
    {
        ...
        uint32_t mark = p->depth*COUNT + (uint32_t)(p->next - p->begin()); /// depth 多少个page  COUNT 满容量的page 4096 / 8 , 加上当前的page next - begin（） 的地址
...
void 
objc_object::sidetable_clearDeallocating()///回收
{
    SideTable& table = SideTables()[this];
...
#if __OBJC2__
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {///是否有自定义allocwithzone
...
+ (Class)superclass {
    return self->superclass;///返回当前类父类的实力变量
}
...
+ (BOOL)isMemberOfClass:(Class)cls {//mateClass
    return object_getClass((id)self) == cls;
}

- (BOOL)isMemberOfClass:(Class)cls {///只能是当前类
...
- (BOOL)isKindOfClass:(Class)cls {///和 isSubclassOfClass 一样 父类
...
- (BOOL)_isDeallocating {
    return ((id)self)->rootIsDeallocating();///判断对象是否在析构
}
...
- (BOOL)allowsWeakReference { 
    return ! [self _isDeallocating];  ///return NO
}

- (BOOL)retainWeakReference { 
    return [self _tryRetain]; ///return NO
}
