...
static inline void *tls_get_direct(tls_key_t k) ///typedef unsigned long __darwin_pthread_key_t; 这个key就是一个 无符号的z长整形
{ 
    assert(is_valid_direct_key(k)); ///key weii 41 42 43 通过

    if (_pthread_has_direct_tsd()) {///真机
        return _pthread_getspecific_direct(k);///获得d该线程在k slot上的数据 返回应该是一个地址 
    } else {///模拟器
        return pthread_getspecific(k);
    }
}
...
class recursive_mutex_tt : nocopy_t {
  ...
  public:
    constexpr recursive_mutex_tt() : mLock(PTHREAD_RECURSIVE_MUTEX_INITIALIZER) {
